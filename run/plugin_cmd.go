...
// buildInfoFile is the generated file that stamps AIGatewayBuildInfo into a
// plugin's main package during `aigateway plugin build`.
const buildInfoFile = "zz_aigateway_buildinfo.go"
...
// runPluginCommand dispatches `aigateway plugin <subcommand> ...`. Usage errors
...
// binary's build flags, adding a generated AIGatewayBuildInfo declaration
...
// package-level AIGatewayBuildInfo.
...
func buildInfoSource(info pluginapi.BuildInfo) string {
	return fmt.Sprintf(`// Code generated by aigateway plugin build. DO NOT EDIT.

package main

import "github.com/nexusrun/nexus_aigateway/pluginapi"

// AIGatewayBuildInfo records the toolchain this plugin was built with. AIGateway
// reads it to explain a refused load.
var AIGatewayBuildInfo = pluginapi.BuildInfo{GoVersion: %q, PluginAPIVersion: %q}
...
func writeBuildInfoOverlay(dir string, info pluginapi.BuildInfo) (string, func(), error) {
	tmp, err := os.MkdirTemp("", "aigateway-plugin-build-")
	if err != nil {
		return "", nil, err
	}
...
func writeManifest(w io.Writer, l pluginload.Loaded) {
	...
	if l.BuildInfo == (pluginapi.BuildInfo{}) {
		fmt.Fprintf(tw, "built with\t- (no AIGatewayBuildInfo; build with `aigateway plugin build`)\n")
	} else {
...
func instancesLabel(single bool) string {
	if single {
		return "one (AIGatewayPlugin is a variable)"
	}
	return "many (AIGatewayPlugin is a constructor)"
}
