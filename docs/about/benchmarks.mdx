---
title: "Benchmarks"
description: "A short, up-to-date summary of AIGateway benchmark results, with a link to the full write-up and the tooling to reproduce it."
icon: "gauge"
...
This page is a short reference for our latest public benchmark: AIGateway against
...
[ai-gateway-reproducible-benchmark](https://github.com/nexusrun/ai-gateway-reproducible-benchmark)
...
[AI Gateway Benchmark 2026: AIGateway vs LiteLLM, Portkey & Bifrost](https://aigateway.nexusai.run/blog/benchmarking-ai-gateways-aigateway-litellm-portkey-bifrost-june-2026/?utm_source=aigateway_docs).
...
- One gateway at a time, in Docker, on an AWS `c7i.large` (2 vCPU, 4 GiB), each
  from its latest public image (`nexusrun/nexus_aigateway`, `litellm/litellm:main-stable`,
...
- Fair config: retries off for everyone, AIGateway's circuit breaker off, and
  LiteLLM run at its recommended one worker per CPU core.

## At a glance

AIGateway came out ahead on every operational signal most teams care about:
...
| --- | --- | --- | --- | --- | --- | --- |
| **AIGateway** | **`2.1`** | **`7.8`** | **`4,200`** | **`60 MB`** | **`14 MB`** | **`0.76 s`** |
...
- AIGateway has both the lowest median (`2.1 ms`) and the tightest tail (`7.8 ms`).
...
[nexusrun/ai-gateway-reproducible-benchmark](https://github.com/nexusrun/ai-gateway-reproducible-benchmark),
...
git clone https://github.com/nexusrun/ai-gateway-reproducible-benchmark.git
...
for a quicker, noisier run. `AIGATEWAY_SOURCE=../aigateway` benchmarks a local
...
```bash
AIGATEWAY_IMAGE=nexusrun/nexus_aigateway:0.1.79 \
...
[`docs/about/benchmark-tools/`](https://github.com/nexusrun/nexus_aigateway/tree/main/docs/about/benchmark-tools).
...
[full post](https://aigateway.nexusai.run/blog/benchmarking-ai-gateways-aigateway-litellm-portkey-bifrost-june-2026/?utm_source=aigateway_docs).
