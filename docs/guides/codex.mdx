---
title: "AIGateway & Codex"
description: "Put AIGateway between Codex and your models: keep billing on your ChatGPT subscription, or route Codex to any other provider."
icon: "code-xml"
keywords: ["Codex", "OpenAI Codex", "ChatGPT subscription", "coding agent", "setup guide"]
---

AIGateway is a good fit for Codex because Codex already targets the OpenAI
Responses API.

`Codex -> AIGateway -> upstream`

Codex talks to AIGateway with a AIGateway master key, so every request shows up in
...
- Choose a AIGateway master key, for example `change-me`.

To bill Codex to your ChatGPT plan, sign in once so Codex writes its token
file, then hand that token to AIGateway:
...
Optional: check that AIGateway answers a plain Responses request before involving
...
## 2. Configure Codex to use AIGateway

Add a Responses-based provider to `~/.codex/config.toml`:

```toml
model_provider = "aigateway"
model = "gpt-5.6-sol"   # or any model your AIGateway serves
...
Then export the AIGateway master key for that provider:

```bash
export AIGATEWAY_API_KEY=change-me
```

<Note>
  Codex requires this variable even when you are signed in with ChatGPT, and it
  carries the AIGateway master key — not an OpenAI key. Without it the provider
  fails to start.
</Note>
...
```bash
AIGATEWAY_API_KEY=change-me codex exec \
  -c model_provider=aigateway \
  -c 'model_providers.aigateway.name="AIGateway"' \
  -c 'model_providers.aigateway.base_url="http://localhost:8080/v1"' \
  -c 'model_providers.aigateway.env_key="AIGATEWAY_API_KEY"' \
  -c 'model_providers.aigateway.wire_api="responses"' \
...
This lets you confirm that Codex is reaching AIGateway and inspect the full
request and response trail. From the same dashboard, you can keep following
your AIGateway traffic and usage.
...
This guide was validated against a local AIGateway instance and Codex CLI
`0.147.0`. Local validation confirmed:

- `codex exec` returned `ok` through `Codex -> AIGateway -> ChatGPT subscription`
...
- the custom `aigateway` provider failed without its `env_key`, because Codex
