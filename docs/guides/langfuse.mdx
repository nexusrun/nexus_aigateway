---
title: "AIGateway & Langfuse"
description: "Send AIGateway gateway traces to Langfuse over OTLP, capture full prompts with the Langfuse SDK pointed at AIGateway, and stitch both into one trace."
icon: "telescope"
...
analytics, prompt management, and evals. It pairs with AIGateway in two
complementary ways:

- **Gateway traces.** AIGateway's [OpenTelemetry exporter](/guides/opentelemetry)
...
- **Application traces.** The Langfuse SDK's OpenAI drop-in wrapper, pointed
  at AIGateway as its base URL. Langfuse records full prompts, completions, and
  token usage; AIGateway supplies the routing, failover, caching, and its own
...
`App (Langfuse SDK) -> AIGateway -> OpenAI/Anthropic/Gemini/...` with traces
...
Then enable AIGateway's exporter:
...
| Variable | Why |
| --- | --- |
| `OTEL_ENABLED` | AIGateway's export switch; off by default. |
...
<Warning>
  ...
  an `https://` endpoint unless Langfuse runs on the same host; AIGateway warns
...
`gen_ai.request.model`, `gen_ai.provider.name`, and `aigateway.provider.name`
...
gateway in telemetry — they stay in AIGateway's [audit log](/features/audit-logging);
...
on this path — AIGateway's own [cost tracking](/features/cost-tracking) has
...
OpenAI wrapper and point it at AIGateway — two changed lines in an existing
OpenAI-SDK app:
...
client = OpenAI(
    base_url="http://localhost:8080/v1",  # AIGateway
    api_key="<aigateway-api-key>",
)

response = client.chat.completions.create(
    model="anthropic/claude-haiku-4-5-20251001",  # any model AIGateway exposes
...
base URL is AIGateway, one client reaches every configured provider through
...
AIGateway's failover, caching, budgets, and audit trail. The
...
a `traceparent` header with the request and AIGateway parents its spans under
the caller's trace.
...
with the prompt, AIGateway's server span, and the provider generation — what
...
<Note>
  ...
  capture through AIGateway, and trace-context propagation merging both.
</Note>
...
| Symptom | Fix |
| --- | --- |
| No traces appear in Langfuse | Check the `Authorization` header is `Basic` + base64 of `pk-lf-...:sk-lf-...` (not the raw keys), and the endpoint ends in `/api/public/otel`. AIGateway logs `opentelemetry enabled` at startup when the exporter is on. Exports are batched — allow a few seconds. |
...
| Gateway generations show no prompt, tokens, or cost | By design: AIGateway never exports request content or usage in telemetry. Prompts and costs live in the [audit log](/features/audit-logging) and [cost tracking](/features/cost-tracking); for prompts in Langfuse, use the SDK path. |
