---
title: "AIGateway & Caddy"
description: "Use Caddy's forward_auth with AIGateway's key verification endpoint to reject unauthenticated traffic at the edge and gate other services on AIGateway API keys."
icon: "shield-check"
...
asks another service whether a request may proceed. Point it at AIGateway's
[`GET /v1/auth/verify`](/advanced/api-endpoints#key-verification) and Caddy
gates traffic on AIGateway API keys — without a copy of those keys anywhere in
the proxy.
...
- **Reject unauthenticated traffic at the edge.** Requests without a usable key
  ...
  connections to it or land in its audit log. AIGateway still authenticates every
  request it serves; the edge check is a filter in front, not a replacement.
- **Gate other services on AIGateway keys.** An internal dashboard, a docs site,
  or a metrics endpoint on the same host can require a valid AIGateway key
...
AIGateway's `401` body is its normal OpenAI-shaped error envelope, so a client
that already understands AIGateway errors sees a familiar shape from the proxy.
...
## Gate another service on AIGateway keys

The upstream here is any app you want behind AIGateway's keys.
...
:8090 {
	forward_auth 127.0.0.1:8080 {
		uri /v1/auth/verify
		copy_headers X-Aigateway-Auth-User
	}
...
`forward_auth` sends the incoming request's own headers to AIGateway, so the
...
`copy_headers X-Aigateway-Auth-User` forwards AIGateway's answer about *who* the
...
To put the same check in front of AIGateway itself, verify against the gateway
and proxy to it:
...
AIGateway's own authentication is already the only gate.
...
would be rejected by the edge gate even though AIGateway serves them
unauthenticated.

<Warning>
  **The gate is only as strong as the gateway's own authentication.** On a
  AIGateway instance with no master key and no managed keys — the unsafe
  ...
  `AIGATEWAY_MASTER_KEY` or issue [managed keys](/features/users) before relying
...
- Enabling the endpoint exposes a key oracle: anyone who can reach it can test
  whether a key is valid. Expose it only where you need it, and keep AIGateway's
...
- Caddy adds `X-Forwarded-Method` and `X-Forwarded-Uri` to the verification
  subrequest. AIGateway ignores both; the check is about the credential, not the
