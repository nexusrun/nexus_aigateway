---
title: "AIGateway & Open WebUI"
description: "Give Open WebUI one connection to every provider through AIGateway, and get audit logs, per-user cost tracking, and metrics for every chat."
...
OpenAI connection at AIGateway fixes that: one connection, and every chat is
...
AIGateway into Open WebUI's model picker.

Flow:

`Open WebUI chat -> Open WebUI backend -> AIGateway -> OpenAI/Anthropic/Gemini/...`
...
## 1. Run AIGateway next to Open WebUI

Run both services on one Docker network so Open WebUI can reach AIGateway by
container name:
...
services:
  ...
  openwebui:
    ...
    environment:
      OPENAI_API_BASE_URL: http://aigateway:8080/v1
...
| Variable | Why |
| --- | --- |
| `OPENAI_API_BASE_URL` | Sends all OpenAI-API traffic to AIGateway. Keep the `/v1` suffix. |
| `OPENAI_API_KEY` | A AIGateway API key (or `AIGATEWAY_MASTER_KEY`), not a real OpenAI key. |
| `ENABLE_OLLAMA_API` | Off unless you run Ollama; avoids connection errors in the logs. |
| `ENABLE_FORWARD_USER_INFO_HEADERS` | Adds the signed-in user's name, email, id, and role as headers on every model call, which AIGateway turns into labels below. |
...
picker lists exactly what AIGateway exposes — including Anthropic and Gemini
...
<Tip>
  Create a dedicated API key for Open WebUI in the AIGateway dashboard
...
AIGateway. Each call appears in the audit log with the full prompt, response,
...
<Note>
  ...
  Gemini models routed through AIGateway.
</Note>

### Per-user cost tracking

With the two variables from step 1 set, every request carries
`X-OpenWebUI-User-Email` (plus `-Name`, `-Id`, and `-Role`) and AIGateway
...
AIGateway can expose hundreds of models, and Open WebUI lists all of them. To
...
- Or expose fewer providers in AIGateway; the picker follows `GET /v1/models`.
...
calls go through AIGateway too, so they stay logged and priced.

## Embeddings and RAG

Open WebUI's document RAG defaults to a local embedding model. To route
embeddings through AIGateway instead, set the embedding engine to `openai`
under `Admin Panel -> Settings -> Documents` with the same base URL and key
as step 1. AIGateway serves `/v1/embeddings` and prices embedding usage like
...
| Symptom | Fix |
| --- | --- |
| Connection error when verifying the OpenAI API connection | Open WebUI cannot resolve the base URL. Use the Docker service name (`aigateway`), not `localhost`; `localhost` inside the container is the container itself. |
| `401 Unauthorized` in AIGateway logs | The connection key does not match a AIGateway API key or `AIGATEWAY_MASTER_KEY`. |
| Model picker is empty | AIGateway has no provider configured. Check `GET /v1/models` and provider environment variables, then refresh the connection under `Admin Panel -> Settings -> Connections`. |
| No per-user labels in usage | `ENABLE_FORWARD_USER_INFO_HEADERS` is off, or `TAGGING_HEADER_1` is not set in AIGateway. Both are needed. |
