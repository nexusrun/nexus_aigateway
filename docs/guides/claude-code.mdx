---
title: "AIGateway & Claude Code"
description: "Step-by-step guide for routing Claude Code through AIGateway with Anthropic passthrough."
icon: "terminal"
keywords: ["Claude Code", "Anthropic passthrough", "coding agent", "setup guide"]
---

AIGateway can sit in front of Claude Code so every request goes through your own
gateway first.

Flow:

`Claude Code -> AIGateway -> Anthropic`

## Before you start

- Install Claude Code on your machine.
- Choose a AIGateway master key, for example `change-me`.
- Make sure AIGateway has an Anthropic upstream credential.

<Note>
  Claude Code can be routed through AIGateway whether or not you personally use a
  Claude Code subscription. For gateway mode, Claude Code talks to AIGateway with
  `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN`. AIGateway still needs its own
...
3. Copy the key once and set it for AIGateway as `ANTHROPIC_API_KEY`.
...
1. Run `claude setup-token` on a machine where Claude Code is logged in to
   your subscription.
2. Copy the generated `sk-ant-oat01-...` token and set it for AIGateway as
   `ANTHROPIC_API_KEY`.

AIGateway detects the `sk-ant-oat` prefix and authenticates upstream with the
...
<Warning>
  ...
  This setup works because Claude Code remains the client and AIGateway relays
...
Start AIGateway with a master key and an Anthropic provider key:
...
## 3. Configure Claude Code to use AIGateway

Point Claude Code at AIGateway:
...
If AIGateway is not running on your local machine, replace `localhost:8080` with
...
## 5. Check the traffic in AIGateway
...
This is the easiest place to confirm that Claude Code is reaching AIGateway and
to inspect the full request and response trail. From the same dashboard, you
can keep following your AIGateway traffic and usage.
...
- a AIGateway branch exposing `/p/{provider}/v1/...`
- Claude Code `2.1.72`
...
- `claude` returned `ok` when pointed at AIGateway
