---
title: "AIGateway & Kimi Code CLI"
description: "Route Kimi Code CLI through AIGateway without an api.json registry URL, configure a model profile, and verify prompts and tool calls."
icon: "terminal"
keywords: ["Kimi Code", "Kimi Code CLI", "Kimi CLI", "api.json", "coding agent", "setup guide"]
---

Kimi Code CLI can use AIGateway as an OpenAI-compatible provider. You configure
the AIGateway base URL, a dedicated managed API key, and at least one model in
Kimi's `~/.kimi/config.toml` file.

Flow:

`Kimi Code CLI -> AIGateway -> upstream model provider`
...
AIGateway exposes its model catalog at `/v1/models`, but that response uses the
...
- Choose a AIGateway master key for gateway administration, for example
  `change-me`.
- Make sure AIGateway has an upstream credential for the model you want to use.
...
AIGateway and is used only for the upstream request.
...
<Warning>
  Use this managed key in Kimi, not `AIGATEWAY_MASTER_KEY`. A managed key without
...
List the models visible through AIGateway:
...
response so AIGateway routes the request to the intended provider.
...
```toml
default_model = "aigateway/gpt-5-mini"
...
The alias `aigateway/gpt-5-mini` is local to Kimi. The `model` value is the
exact selector Kimi sends to AIGateway.
...
<Warning>
  ...
  `api_key` must contain the dedicated AIGateway managed key, not the master key
  or an upstream provider key. Restrict access with
  `chmod 600 ~/.kimi/config.toml`.
</Warning>

Use the `context_window` value returned by AIGateway as `max_context_size` when
...
If AIGateway runs on another host, replace `localhost:8080`. Keep `/v1` in the
...
`POST /v1/chat/completions` and that AIGateway is routing them to the selected
...
`~/.kimi/config.toml` as shown above. AIGateway does not need a registry
...
- `openai/gpt-5-mini` through AIGateway's OpenAI-compatible Chat Completions API
...
- `kimi --quiet -p` returned `ok` through `Kimi Code CLI -> AIGateway -> OpenAI`
...
- no AIGateway application change or `api.json` endpoint was required
