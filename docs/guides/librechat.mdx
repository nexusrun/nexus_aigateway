---
title: "AIGateway & LibreChat"
description: "Give LibreChat one custom endpoint to every provider through AIGateway, and get audit logs, per-user cost tracking, and metrics for every chat."
icon: "message-circle"
...
through custom endpoints. Pointing one custom endpoint at AIGateway gives every
LibreChat user all your providers behind a single connection, and every chat
is logged, priced, and attributed to the signed-in LibreChat user in AIGateway.

Flow:

`LibreChat chat -> LibreChat backend -> AIGateway -> OpenAI/Anthropic/Gemini/...`

What you get:

- Full request and response bodies for every chat in the AIGateway audit log.
...
- Failover, rate limits, budgets, and response caching applied to chat
  traffic, when you configure them in AIGateway.

## 1. Run AIGateway next to LibreChat
...
endpoints:
  custom:
    - name: "AIGateway"
      ...
      baseURL: "http://aigateway:8080/v1"
      headers:
...
Then run AIGateway, LibreChat, and MongoDB on one Docker network so LibreChat
can reach AIGateway by container name:
...
| Setting | Why |
| --- | --- |
| `baseURL` | Sends the endpoint's traffic to AIGateway. Keep the `/v1` suffix. |
| `apiKey` / `AIGATEWAY_API_KEY` | A AIGateway API key (or `AIGATEWAY_MASTER_KEY`), not a real provider key. |
| `headers` | Adds the signed-in user's email to every model call via LibreChat's `{{LIBRECHAT_USER_EMAIL}}` template, which AIGateway turns into labels below. |
| `models.fetch` | Fills the model picker from AIGateway's `GET /v1/models` instead of the hardcoded `default` list. |
| `titleModel` | LibreChat titles conversations with `gpt-3.5-turbo` by default, which AIGateway does not expose. Set a model id AIGateway serves. |
| `TAGGING_HEADER_1` | Tells AIGateway to record that header's value as a label on every request. |
...
LibreChat admin), and the `AIGateway` endpoint appears in the model picker with every model AIGateway exposes —
including Anthropic and Gemini models and any
[virtual models](/features/virtual-models).

<Tip>
  Create a dedicated API key for LibreChat in the AIGateway dashboard
...
## 2. Chat, and see every call in AIGateway
...
generation LibreChat runs after the first exchange, all pass through AIGateway.
...
<Note>
  ...
  file embedding, all routed through AIGateway.
</Note>

### Per-user cost tracking

With the `headers` template from step 1, every request carries
`X-LibreChat-User-Email` and AIGateway records its value as a label. The
...
AIGateway can expose hundreds of models, and `fetch: true` lists all of them.
...
Or expose fewer providers in AIGateway; `fetch: true` follows `GET /v1/models`.

## Embeddings and RAG

LibreChat's file uploads are embedded by its separate RAG API, which speaks
the OpenAI embeddings API and can point at AIGateway too. Add to the compose
file:
...
```yaml
  rag_api:
    ...
    environment:
      ...
      RAG_OPENAI_BASEURL: http://aigateway:8080/v1
...
embedded through AIGateway's `/v1/embeddings`, so embedding usage is logged
...
| Model picker shows only the `default` list | `fetch: true` could not reach AIGateway or the key was rejected. Use the Docker service name (`aigateway`) in `baseURL`, not `localhost`, and check AIGateway logs for `401`. |
| "Outdated Config version" warning at startup | Bump the `version:` field in `librechat.yaml` to the version the warning suggests. |
| Conversations stay titled "New Chat", with `404` errors in AIGateway logs | `titleModel` is unset, so LibreChat asks for `gpt-3.5-turbo`. Set `titleModel` to a model AIGateway exposes. |
| No per-user labels in usage | The `headers` block is missing from the endpoint, or `TAGGING_HEADER_1` is not set in AIGateway. Both are needed. |
| File uploads fail | `RAG_API_URL` is not set or the RAG API is not running. See [Embeddings and RAG](#embeddings-and-rag). |
| Same answer repeats for identical prompts | AIGateway's [response cache](/features/cache) replays identical requests. This is usually what you want; disable the cache if not. |

## Notes

- AIGateway [failover](/features/failover) and [budgets](/features/budgets)
