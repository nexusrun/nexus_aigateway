---
title: "AIGateway & Cursor"
description: "Route Cursor's OpenAI BYOK model traffic through AIGateway, configure a public HTTPS base URL, and understand which Cursor features bypass the gateway."
icon: "mouse-pointer-click"
keywords: ["Cursor", "Cursor IDE", "BYOK", "OpenAI base URL", "AI gateway", "setup guide"]
---

Cursor can use AIGateway through its **OpenAI API Key** and **Override OpenAI
Base URL** settings. This is an available but limited integration path, not a
gateway for every Cursor feature.

Flow:

`Cursor IDE -> Cursor backend -> AIGateway -> upstream model provider`

<Warning>
  AIGateway must be available at a public HTTPS URL. Cursor routes requests
  through its backend for final prompt building, so
  `http://localhost:8080/v1` is not a reliable Cursor base URL. Put a deployed
  AIGateway instance behind TLS, or use a secure tunnel for a temporary test.
</Warning>

## Cursor subscription and AIGateway are separate

A paid Cursor subscription can be used alongside AIGateway. It provides the
...
It does not provide upstream model credit to AIGateway:
...
| OpenAI BYOK with AIGateway base URL | AIGateway, then its configured provider | The provider account configured in AIGateway |
...
<Warning>
  ...
  as AIGateway provider credit. Cursor does not expose subscription models as a
  general OpenAI-compatible inference API. The Cursor SDK and Cloud Agents API
  run Cursor agents; they are not raw model endpoints that AIGateway can use as
  an upstream provider.
</Warning>

When AIGateway is selected, you therefore pay for the Cursor plan and for model
usage on the provider account behind AIGateway. Teams and Enterprise customers
also pay Cursor's token rate on eligible third-party BYOK requests.

## What goes through AIGateway

| Cursor feature | AIGateway gateway status |
| --- | --- |
...
it before using a Cursor-hosted model that should not go through AIGateway.

## Before you start

- Deploy AIGateway at a public HTTPS URL, such as
  `https://aigateway.example.com`.
- Use a paid Cursor plan for Agent and Edit with the AIGateway BYOK route.
- Configure at least one upstream provider and model in AIGateway.
- Create a dedicated AIGateway managed API key for Cursor at
...
<Note>
  ...
  AIGateway key rather than `AIGATEWAY_MASTER_KEY`.
</Note>

## 1. Run AIGateway

This example starts AIGateway with an OpenAI upstream. In production, terminate
...
`AIGATEWAY_MASTER_KEY` administers AIGateway. Do not enter it in Cursor after you
...
AIGateway. Replace `sk_gom_...` with the dedicated key created for Cursor.

```bash
curl -s https://aigateway.example.com/v1/models \
...
```bash
curl -s https://aigateway.example.com/v1/chat/completions \
  -H "Authorization: Bearer sk_gom_..." \
...
`gpt-4.1-mini`. Cursor forwards that model name unchanged to AIGateway.

AIGateway normally lists provider-qualified selectors such as
...
You can also add a custom name in Cursor and use the same name as a AIGateway
...
<Steps>
  ...
  <Step title="Set the OpenAI API key">
    Find **OpenAI API Key**, enable it, and enter the dedicated AIGateway managed
    key.
  </Step>
  <Step title="Override the OpenAI base URL">
    Enable **Override OpenAI Base URL** and enter
    `https://aigateway.example.com/v1`. Keep `/v1`; do not append
...
```text
Create a file named aigateway-cursor-test.txt containing exactly tool-ok
```

Confirm the edit before accepting it. Sign in to AIGateway's dashboard separately
...
<Warning>
  ...
  the AIGateway audit response before changing the gateway.
</Warning>
...
Use a public HTTPS URL. Cursor's backend must be able to reach AIGateway. A local
address, private IP, or self-signed TLS certificate will not work reliably.

### `401 Unauthorized`

Make sure the value in **OpenAI API Key** is the dedicated AIGateway key and that
the key has not been revoked. Do not enter an upstream provider key in Cursor.

### `Model not found`

Cursor forwards the selected model name to AIGateway. Copy the exact selector
...
Upgrade Cursor, retry with a standard OpenAI chat model, and inspect AIGateway's
...
### Images do not reach AIGateway

Upgrade Cursor. Cursor fixed the custom-base-URL image routing issue in release
`3.9.16` in June 2026. Older releases can validate the AIGateway key against
OpenAI directly and fail before the request reaches AIGateway.

## Security and privacy

This setup does not create a direct private connection from the Cursor desktop
app to AIGateway. Cursor's backend remains in the request path for prompt
construction and context retrieval. It also does not make Cursor Tab or other
Cursor-hosted features use AIGateway.

For an internet-facing AIGateway deployment:
...
AIGateway already exposes the required `/v1/models`, `/v1/chat/completions`, and
`/v1/responses` endpoints, so no AIGateway application change is required for the
documented path. Local AIGateway validation with `openai/gpt-4.1-mini` confirmed
...
AIGateway endpoints and model/tool behavior were tested separately; repeat
