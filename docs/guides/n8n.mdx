---
title: "AIGateway & n8n"
description: "Route every n8n AI Agent, LLM Chain, and OpenAI node call through AIGateway with one credential, and get audit logs, cost tracking, and metrics for all of your workflows."
icon: "workflow"
...
at AIGateway fixes that without any community node: one **OpenAI** credential
with AIGateway's base URL, and every workflow's model call is logged, priced,
and exported as metrics. The same credential also lets n8n's OpenAI nodes use
Anthropic, Gemini, and any other provider configured in AIGateway.

Flow:

`n8n workflow -> OpenAI Chat Model / OpenAI node -> AIGateway -> OpenAI/Anthropic/Gemini/...`

What you get:

- Full request and response bodies for every AI call in the AIGateway audit log.
...
## 1. Run AIGateway next to n8n

Run both services on one Docker network so n8n can reach AIGateway by container
name:

```yaml docker-compose.yml
services:
  aigateway:
    image: nexusrun/nexus_aigateway
    environment:
...
Confirm AIGateway lists your models:
...
<Tip>
  Create a dedicated API key for n8n in the AIGateway dashboard
...
| Field | Value |
| --- | --- |
| API Key | The AIGateway API key (or `AIGATEWAY_MASTER_KEY`) |
| Organization ID | Leave empty |
| Base URL | `http://aigateway:8080/v1` |
| Add Custom Header | Optional, see [Attribute workflows with labels](#attribute-workflows-with-labels) |

n8n tests the credential against `GET /v1/models` when you save it. AIGateway
serves that endpoint, so the test should pass and the model picker in the
**OpenAI Chat Model** node lists exactly the models AIGateway exposes, including
Anthropic and Gemini models and any [virtual models](/features/virtual-models).

<Note>
  Use the Docker service name (`aigateway`) when both run in containers.
  `http://localhost:8080/v1` only works when n8n runs on the host machine.
</Note>

## 3. Use it in a workflow

Any node that accepts the OpenAI credential now goes through AIGateway:
...
Because the list comes from AIGateway, you can select
...
the AIGateway dashboard: the call appears in the audit log with the full prompt,
...
it is **on by default** for new nodes. Both paths work through AIGateway:

| Setting | AIGateway endpoint | Notes |
| --- | --- | --- |
...
| Off | `/v1/chat/completions` | Works with every provider and every AIGateway version. |
...
loudly rather than translated, or if you run a AIGateway release older than the
...
declare it in AIGateway:

1. In AIGateway, set `TAGGING_HEADER_1=X-N8N-Workflow` (or add it under
...
(`X-N8n-Workflow`); AIGateway matches it case-insensitively. Create one
credential per workflow or team if you want separate labels; they can share the same AIGateway API key. See
...
Point it at `http://aigateway:8080` (no `/v1`) with a AIGateway API key to route
native `/v1/messages` traffic through AIGateway; the credential test and AI
...
| `401 Unauthorized` in AIGateway logs | The n8n credential key does not match a AIGateway API key or `AIGATEWAY_MASTER_KEY`. |
| Model picker is empty | AIGateway has no provider configured for that model. Check `GET /v1/models` and provider environment variables. |
| `Cannot read properties of undefined (reading 'map')` with **Use Responses API** on | AIGateway releases before the `annotations` fix omitted `annotations: []` from `output_text` parts, which n8n's OpenAI client requires. Upgrade AIGateway, or turn **Use Responses API** off. |
| Same error persists right after upgrading | AIGateway's response cache is replaying a body stored before the upgrade. Change the prompt or clear the cache. |
| Responses API errors on Anthropic or Gemini | Drop `conversation_id` and hosted tools; those are not translatable. |
| Model answers look "off" | Check AIGateway guardrails; a `system_prompt` guardrail rewrites the prompt n8n sends. |

## Notes

- AIGateway [failover](/features/failover) and [caching](/features/cache) apply
...
- Community nodes such as `n8n-nodes-openai-litellm` exist to add gateway
  metadata to n8n requests. They are not required with AIGateway; the built-in
