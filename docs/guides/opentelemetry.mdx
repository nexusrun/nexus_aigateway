---
title: "OpenTelemetry"
description: "Export AIGateway traces and metrics over OTLP with OTEL_ENABLED, configure the collector with standard OTEL_* variables, and see which spans and GenAI metrics are exported."
icon: "network"
keywords: ["OpenTelemetry", "OTLP", "tracing", "traces", "metrics", "observability", "GenAI semantic conventions", "Jaeger", "Grafana Tempo"]
---

AIGateway exports OpenTelemetry traces and metrics for inbound HTTP requests
...
docker run --rm -p 8080:8080 --env-file .env nexusrun/nexus_aigateway
```
...
AIGateway logs `opentelemetry enabled` at startup together with the selected
...
`OTEL_ENABLED` is the only AIGateway-specific switch. Everything else follows
the standard
[OpenTelemetry environment variables](https://opentelemetry.io/docs/specs/otel/configuration/sdk-environment-variables/),
so AIGateway is configured exactly like any other instrumented service. The
...
| `OTEL_SERVICE_NAME` | `service_name` | product name (`aigateway`; `aigateway-pro` in Pro) | `service.name` resource attribute. |
| `OTEL_RESOURCE_ATTRIBUTES` | `resource_attributes` | N/A | Extra resource attributes such as `deployment.environment`. |
...
| `OTEL_EXPORTER_OTLP_HEADERS` | `headers` | N/A | Headers such as `authorization` for hosted backends. |
...
| `OTEL_TRACES_SAMPLER_ARG` | `sampler_arg` | N/A | Sampler argument, e.g. `0.1` for 10%. |
| `OTEL_PROPAGATORS` | `propagators` | `tracecontext,baggage` | Comma-separated: `tracecontext`, `baggage`, `b3`, `b3multi`, `jaeger`, `ottrace`, or `none`. |
| `OTEL_METRIC_EXPORT_INTERVAL` | N/A | `60000` | Metric export interval in milliseconds. |
...
```bash Environment
OTEL_ENABLED=true
OTEL_SERVICE_NAME=aigateway-production
...
opentelemetry:
  enabled: true
  service_name: aigateway-production
  resource_attributes:
...
<Warning>
  ...
  AIGateway logs a warning at startup when headers are configured and the
...
| `aigateway.provider.name` | `openai-eu` | The exact provider name from your AIGateway configuration. |
...
`aigateway.client.empty_responses` counts buffered chat and Responses API calls
...
`aigateway --reload` (SIGHUP) rebuilds the OpenTelemetry pipeline with the
