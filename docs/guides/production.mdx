...
description: "A production-grade deployment checklist for AIGateway: storage backends, credential handling, air-gapped installs, multi-replica behavior, retention, and hardening."
icon: "server-cog"
keywords: ["production", "deployment", "self-hosted", "air-gapped", "offline", "Kubernetes", "hardening", "security"]
---

This page collects the best practices for running AIGateway in production. The
...
AIGateway is a single static Go binary with no runtime dependencies. It is built
...
The practical consequence is that AIGateway fits comfortably where size and cold
...
AIGateway persists audit logs, usage records, budgets, virtual models, managed API
...
SQLite is the default so AIGateway works with zero configuration, and it is a real
...
<Warning>
  **The Docker image does not declare a volume.** The default database path
  resolves to `/app/data/aigateway.db` inside the image, which is an image-layer
  ...
  repository's `docker-compose.yaml` mounts the `aigateway_data` volume there.
</Warning>

The default SQLite path is `./data/aigateway.db` when a `./data` directory already
...
[Where does AIGateway store its data?](/about/faq#where-does-aigateway-store-its-data).

### Redis

Redis is optional. It backs the shared model catalog snapshot and the exact
[response cache](/features/cache). Without it, AIGateway uses an in-process cache
...
AIGateway takes provider credentials from environment variables, from
...
AIGateway does not support a `<VAR>_FILE` convention for reading secrets from
...
- **Set `AIGATEWAY_MASTER_KEY`.** If it is empty and no managed keys exist, every
...
AIGateway does not terminate TLS. It serves plain HTTP and expects a reverse proxy
...
**AIGateway runs fully air-gapped.** There is no telemetry, no analytics, and no
...
```bash
AIGATEWAY_OFFLINE=true
```

| Call | What it does | Default |
| --- | --- | --- |
| [Update check](/advanced/version-awareness) at `AIGATEWAY_VERSION_CHECK_URL` | Daily fetch of the latest release number, with an anonymous install id | on |
...
`AIGATEWAY_OFFLINE=true` (or `offline: true` in `config.yaml`) disables the
...
`AIGATEWAY_VERSION_CHECK_ENABLED=false` and `MODEL_LIST_URL=off`.
...
1. **Ship the catalog as a file.** Download `models.min.json` outside the air
   ...
   fetched, so it stays active under `AIGATEWAY_OFFLINE=true`:

   ```bash
   MODEL_LIST_URL=/etc/aigateway/models.json   # or file:///etc/aigateway/models.json
   ```
...
2. **Mirror it.** Serve the file from an internal HTTP host and set
   `MODEL_LIST_URL` to that URL. Leave `AIGATEWAY_OFFLINE` unset and disable the
   update check separately, or point `AIGATEWAY_VERSION_CHECK_URL` at an
   internal mirror too.
...
AIGateway exposes liveness and readiness separately, and the binary can probe
itself, which works in a distroless image with no shell or `curl`.

| Purpose | HTTP | CLI | Checks |
| --- | --- | --- | --- |
| Liveness | `GET /health` | `aigateway --health` | Nothing. Confirms the listener answers. |
| Readiness | `GET /health/ready` | `aigateway --ready` | Storage (503 if down), Redis (degraded, still 200). |

Point Kubernetes readiness at `/health/ready`, not `/health`. Run the gateway
as a non-root user on a read-only filesystem, keep SQLite on a persistent
volume, and use PostgreSQL or MongoDB before scaling beyond one replica.
...
- **`aigateway_circuit_breaker_state`** is the gauge to alert on for provider
...
- **Restrict the database file.** The SQLite data directory is created `0755`
  ...
  world-readable. Run AIGateway as a dedicated user and tighten permissions,
...
- Set `AIGATEWAY_MASTER_KEY` to a strong value.
...
- Enable metrics and alert on `aigateway_circuit_breaker_state` and on
  `usage log buffer full`.
- For air-gapped installs, set `AIGATEWAY_OFFLINE=true`, then ship the model
