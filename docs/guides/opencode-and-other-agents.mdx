---
title: "AIGateway & Coding Agents"
description: "Connect OpenCode, Cline, Roo Code, and other OpenAI-compatible coding agents to AIGateway through the standard /v1 endpoint and a single gateway key."
icon: "bot"
keywords: ["OpenCode", "coding agents", "OpenAI-compatible", "setup guide"]
---

Most coding agents outside Claude Code are easiest to connect to AIGateway
through the standard OpenAI-compatible API.

Flow:

`OpenCode / Cline / Roo Code / Kilo Code -> AIGateway -> your upstream providers`

## Before you start

- Choose a AIGateway master key, for example `change-me`.
- Start AIGateway with at least one upstream provider key.
- Use AIGateway standard `http://localhost:8080/v1` unless your client
  specifically requires a full endpoint URL.

## 1. Run AIGateway

Start AIGateway with a master key and at least one upstream provider:
...
<Note>
  ...
  target models use. The important part is that AIGateway has at least one
...
Create an `opencode.json` file with a AIGateway provider:
...
```bash
opencode run -m aigateway/gpt-4.1-mini \
...
This is the quickest way to confirm that your coding agent is reaching AIGateway
and to inspect each interaction. From the same dashboard, you can keep
following your AIGateway traffic and usage.
...
- local validation against a AIGateway instance on `http://localhost:8080`
...
- AIGateway dashboard audit logs showed the OpenCode traffic immediately
