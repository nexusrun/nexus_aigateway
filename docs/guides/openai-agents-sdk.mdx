---
title: "AIGateway & OpenAI Agents SDK"
description: "Route OpenAI Agents SDK model calls through AIGateway's OpenAI-compatible Responses API."
icon: "bot"
keywords: ["OpenAI Agents SDK", "Responses API", "agents", "Python SDK"]
---

AIGateway can sit in front of the OpenAI Agents SDK when you want gateway keys,
budgets, audit logs, model routing, and usage tracking around agent runs.

Flow:

`OpenAI Agents SDK -> AIGateway /v1 -> selected provider`
...
<Note>
  The SDK exports traces to OpenAI by default. If AIGateway is your only OpenAI
  client endpoint, disable tracing or configure a separate OpenAI tracing key.
</Note>

## 1. Run AIGateway

Start AIGateway with a master key and an upstream provider key. This example uses
OpenAI upstream:
...
If you use another provider, keep the same AIGateway base URL and choose a model
that your provider exposes through AIGateway.

## 2. Confirm the Responses endpoint

Send one small request through AIGateway:
...
Point the SDK's OpenAI client at AIGateway:
...
want those namespaced AIGateway IDs to reach the gateway unchanged, configure the
...
AIGateway supports the SDK's normal HTTP Responses path:
...
AIGateway also preserves newer Responses input items for native Responses
providers. If a request has to be translated to Chat Completions for a provider
that does not implement Responses natively, AIGateway translates structured
...
AIGateway stores local response snapshots so lifecycle endpoints can work even
when the upstream provider does not support response retrieval. If the SDK sends
`store: false`, AIGateway does not persist that local response snapshot.

## Current limitations

- Responses websocket transport is not implemented. Use the SDK's HTTP/SSE
  transport with AIGateway.
- Hosted tools such as web search, file search, and computer use require a
  ...
  providers such as Anthropic and Gemini reject those tools unless AIGateway adds
  explicit provider capability mapping for them.
- Anthropic does not currently accept translated structured-output
  `response_format` or `text.verbosity` settings. AIGateway rejects those fields
  instead of silently ignoring them.
- `previous_response_id` is forwarded to native Responses providers. For
  chat-translated providers AIGateway resolves it from its stored responses and
