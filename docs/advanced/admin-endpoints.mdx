...
AIGateway ships with admin endpoints **enabled by default**. The goal is simple: you should be able to deploy AIGateway and immediately have visibility into what's happening, no extra services, no separate monitoring stack, no configuration.

The admin layer is split into two independently controllable pieces:

1. **Admin REST API** (`/admin/*`), machine-readable JSON endpoints for usage data, budgets, and model inventory. Protected by `AIGATEWAY_MASTER_KEY` like all other API routes.
...
- **`AIGATEWAY_MASTER_KEY`**: always has admin access.
...
`X-AIGateway-User-Path` header never widens a key's scope.
...
<Warning>
  If your AIGateway instance is publicly accessible, be aware that the dashboard
...
| `start_date` | string | Start date (`YYYY-MM-DD`)                        | N/A |
| `end_date`   | string | End date (`YYYY-MM-DD`)                          | N/A |
| `session_id` | string | Exact detected session id                        | N/A |
| `user_path`  | string | Tracked user-path subtree                        | N/A |
| `model`      | string | Exact model name                                 | N/A |
| `provider`   | string | Provider name or type                            | N/A |
| `label`      | string | Exact request label                              | N/A |
...
The dashboard is a server-rendered HTML page embedded in the AIGateway binary. Access it at:
