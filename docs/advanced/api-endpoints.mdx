---
title: "API Endpoints"
description: "Reference for AIGateway's OpenAI-compatible and Anthropic-compatible endpoints, provider passthrough, and operations routes."
icon: "list-tree"
keywords: ["API endpoints", "OpenAI-compatible API", "routes", "passthrough", "reference"]
---

AIGateway exposes OpenAI-compatible and Anthropic-compatible APIs, provider-native
...
```bash
wscat -H "Authorization: Bearer $AIGATEWAY_API_KEY" \
...
database, `master_key` the bootstrap key from `AIGATEWAY_MASTER_KEY`. `key_id`
