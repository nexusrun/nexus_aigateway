...
description: "How AIGateway preserves provider replay state such as Gemini 3 thought signatures and Anthropic thinking blocks across turns, APIs, and providers"
icon: "fingerprint"
---
...
Such state has no OpenAI-compatible field, so AIGateway carries it in one place:
...
AIGateway stores nothing. The state travels inside the response to the client
...
- **Only the owning provider sees its own vendor.** Before a request leaves
  for a provider, AIGateway drops every vendor object that provider does not
...
<Steps>
  ...
  <Step title="Gemini replies with a function call and a signature">
    AIGateway translates the request into Gemini's native `generateContent`
  ...
  <Step title="AIGateway returns the call with the signature under extra_content">
    ```json
    {
      "choices": [{
        "message": {
  ...
  <Step title="AIGateway replays the signature on the matching Gemini part">
    ```json
    {
      "contents": [
    ...
    Gemini accepts the replay and answers with text. AIGateway returns a normal
    chat completion.
  </Step>
</Steps>
...
member, AIGateway sends Google's documented placeholder instead of surfacing the
...
<Steps>
  ...
  <Step title="AIGateway keeps the blocks under extra_content.anthropic">
