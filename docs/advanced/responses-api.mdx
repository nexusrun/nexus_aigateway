---
title: "Responses API"
description: "Use OpenAI-compatible response creation, lifecycle, input items, and utility endpoints through AIGateway."
...
AIGateway exposes OpenAI-compatible Responses API endpoints under `/v1/responses`.
...
| `DELETE /v1/responses/{id}` | Deletes the stored gateway response. When the provider supports native deletion, AIGateway also forwards the delete request upstream. |
...
AIGateway stores the normalized response body and the normalized input items
...
`aigateway_response_snapshot_store_failures_total` metric.

This enables:

- `GET /v1/responses/{id}` for responses created through AIGateway.
...
- `previous_response_id` on chat-translated providers such as Anthropic and
  Gemini: AIGateway follows the stored response and each response it was chained
  ...
  they need no AIGateway snapshot. An id AIGateway minted for a chat-translated
  ...
  being resolved upstream, while an id AIGateway never stored is still
  forwarded.
...
When a response was not created through the current AIGateway process or response
...
Without a stored response or provider hint, AIGateway checks providers that expose
native Responses lifecycle support. If lifecycle routing is unavailable,
AIGateway returns a compatibility error. If lifecycle routing is available but no
provider can serve the response, AIGateway returns a normal not-found error.

## Input items

AIGateway normalizes stored input into OpenAI-compatible response input items.
...
When the selected provider cannot perform an operation, AIGateway returns an
...
AIGateway uses OpenAI's `invalid_request_error` type for unsupported operations
