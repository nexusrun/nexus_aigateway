---
title: "Swagger UI"
description: "Serve an interactive Swagger UI for AIGateway's API from a binary built with the swagger build tag."
icon: "file-code"
keywords: ["Swagger", "Swagger UI", "OpenAPI", "build tag", "API explorer"]
---

AIGateway can serve an interactive Swagger UI at `/swagger/index.html`. It is
...
[`docs/openapi.json`](https://github.com/nexusrun/nexus_aigateway/blob/main/docs/openapi.json)
in any OpenAPI viewer instead.

## Build with Swagger

```bash
go build -tags=swagger -o bin/aigateway ./cmd/aigateway
```
...
```bash
SWAGGER_ENABLED=true ./bin/aigateway
```
...
<Warning>
`/swagger/*` bypasses `AIGATEWAY_MASTER_KEY` and gateway API key authentication,
...
without the tag, AIGateway logs a warning at startup and serves nothing at
`/swagger/*`.
...
[DEVELOPMENT.md](https://github.com/nexusrun/nexus_aigateway/blob/main/docs/DEVELOPMENT.md)
for details.
