...
description: "How AIGateway checks for a newer release, exactly what the check sends, and how to turn it off."
icon: "refresh-cw"
keywords: ["version", "update", "upgrade", "telemetry", "privacy", "air-gapped"]
---

AIGateway checks once a day whether a newer release exists, so you find out about
...
We use it to see how many deployments are running which version of AIGateway.
...
[GitHub](https://github.com/nexusrun/nexus_aigateway/issues). For anything security-sensitive, use
the contact in [SECURITY.md](https://github.com/nexusrun/nexus_aigateway/blob/main/SECURITY.md).

The check reads one plain-text file:

| Distribution | URL                              | Contents    |
| ------------ | -------------------------------- | ----------- |
| AIGateway      | `(thiswebsite)/version/core.txt` | `X.Y.Z`     |
| AIGateway Pro  | `(thiswebsite)/version/pro.txt`  | `X.Y.Z+pro` |
...
- **The first dashboard visit of each day**, per browser. A cookie named
  `aigateway_version_check` holds `YYYY-MM-DD-{id}` — the day this browser last
...
| ------------------- | ----------- | ----------------------------------- |
| `X-AIGateway-Version` | `0.1.81`    | the release you are running         |
| `X-AIGateway-App`     | `AIGateway`   | `AIGateway` or `AIGateway Pro`          |
| `X-AIGateway-Install` | a UUID      | anonymous per-deployment id, below  |
| `X-AIGateway-Source`  | `scheduled` | `scheduled` or `dashboard`          |
...
hints, and the visit cookie value (`X-AIGateway-Date`).
...
SQLite — the identifier is derived from `AIGATEWAY_MASTER_KEY` with HMAC-SHA256,
...
`AIGATEWAY_OFFLINE=true` turns the check off together with the remote model
...
```bash
AIGATEWAY_VERSION_CHECK_URL=https://releases.internal.example.com/version
```
...
| --------------------------------- | ----------------------------------------- | --------------------------------------- |
| `version_check.enabled`           | `AIGATEWAY_VERSION_CHECK_ENABLED`           | `true`                                  |
| `version_check.url`               | `AIGATEWAY_VERSION_CHECK_URL`               | `https://aigateway.nexusai.run/version` |
| `version_check.interval_hours`    | `AIGATEWAY_VERSION_CHECK_INTERVAL_HOURS`    | `24`                                    |
| `version_check.timeout_seconds`   | `AIGATEWAY_VERSION_CHECK_TIMEOUT_SECONDS`   | `5`                                     |
| `version_check.max_daily_checks`  | `AIGATEWAY_VERSION_CHECK_MAX_DAILY`         | `500`                                   |
