...
AIGateway wraps every upstream provider call with two resilience layers:
...
`{"error": ...}` JSON body. AIGateway detects such payloads and handles them as
...
the dashboard or in the `aigateway_circuit_breaker_state` metric.
...
state is also exported as the `aigateway_circuit_breaker_state` gauge
...
want AIGateway to try a different model or provider when the primary keeps
...
fail, AIGateway tries model2 with its own retry budget. Model1's open breaker
