...
`github.com/nexusrun/nexus_aigateway/pluginapi` package, and three ways to ship a
plugin:

| How | When to use it | Registered by |
| --- | --- | --- |
| **Built in** | Ships with AIGateway: `system_prompt`, `llm_based_altering`, `string_replace`, `header_edit`, `llm_judge`, and the `cheapest_healthy` routing strategy | nothing to do |
...
nothing else from AIGateway.

## Phases

A plugin declares which hooks it implements in `Manifest.Kinds`. AIGateway runs
...
is written). A manifest may declare them; AIGateway validates that the
...
| `warn` | continue; record `Code`, `Message`, and `Detail` in the audit trail and add `X-AIGateway-Guardrail: warn; code=<code>` to the client response | `Code` `guardrail_warning` |
...
ends at `timeout_ms` (and when the client disconnects); AIGateway stops
...
AIGateway builds one plugin value per guardrail definition and calls `Init`
...
| `buffer` | nothing per event; AIGateway collects the whole stream and runs the plugin's `OnResponse` on the assembled completion (a `buffer` plugin must implement `ResponseHook`; loading fails otherwise) | time-to-first-token becomes time-to-last-token |

**Lookbehind** (`transform` only): AIGateway withholds the last `LookbehindChars`
...
call per token. `MinChunkChars` makes AIGateway collect the text deltas of a
...
the default whenever the response chain runs or a plugin sets no cap. While draining, AIGateway sends the SSE comment
`: aigateway-buffering` every 15 s so proxies and clients do not time out; SDKs
...
**Warnings on streams**: when response or stream plugins run, AIGateway commits
...
`X-AIGateway-Guardrail` header as long as buffering finished before the first
...
Edits go through methods, not field assignment, so AIGateway re-encodes only
...
```yaml
plugins:
  search_paths: ["/etc/aigateway/plugins"]   # PLUGINS_SEARCH_PATHS, comma-separated; default: empty
  load:
...
- **Platform**: Linux, macOS, and FreeBSD, and only in a binary built with
  `CGO_ENABLED=1`. The default AIGateway binary and image are static and
  refuse `.so` files with an error that says so. Use `make build-plugins`
  (`bin/aigateway-plugins`) or the `aigateway:<version>-plugins` image
...
- **Exact toolchain**: the plugin must be built with the **same Go version**,
  ...
  `pluginapi` are shared, so internal AIGateway changes never affect a plugin,
  but every AIGateway release and every Go toolchain update (patch releases
...
```sh
aigateway plugin build ./docs/example_plugins/keywordblock -o plugins/keyword_block.so
aigateway plugin inspect plugins/keyword_block.so
...
the `aigateway` binary that runs it, forces `CGO_ENABLED=1`, pins `GOTOOLCHAIN`
to the host's Go version, stamps a `AIGatewayBuildInfo` variable into the
...
```sh
docker build -f Dockerfile.plugins --target plugin-builder -t aigateway-plugin-builder .
docker run --rm -v "$PWD/my-plugin:/src" -v "$PWD/plugins:/out" \
  aigateway-plugin-builder -o /out/my-plugin.so /src
```

`aigateway --version` prints the Go and `pluginapi` versions of the binary.
`docs/example_plugins/README.md` covers building a plugin in its own Go module.

## Writing a plugin

A `package main` that exports `func AIGatewayPlugin() pluginapi.Plugin` is a
...
func AIGatewayPlugin() pluginapi.Plugin { return &wordBlock{} }
...
- Export a **constructor** (`func AIGatewayPlugin() pluginapi.Plugin`) so one
  file can back several instances. A `var AIGatewayPlugin pluginapi.Plugin`
...
- Implement `pluginapi.HealthChecker` (`Health(ctx) error`) when an instance
  ...
  AIGateway probes it off the request path once the instance is built and on
...
- Implement `pluginapi.ContentEditor` (`EditsContent() bool`) when a plugin
  ...
  set to flag instead of rewrite. AIGateway asks a configured instance before
...
`pluginapi/plugintest` tests a plugin without AIGateway:
...
probes, and session pinning stay AIGateway's job.
...
- AIGateway falls back to weighted round robin, logging one warning per
...
`route_fields` the route-scoped ones. A `.so` built with `aigateway plugin
