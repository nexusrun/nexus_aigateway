---
title: "Images API"
description: "OpenAI-compatible image generation and editing through AIGateway, routed by model with the same access rules, budgets, rate limits, and cost tracking as chat."
icon: "image"
...
AIGateway exposes the **OpenAI-compatible image endpoints** for generation and
editing. Clients and SDKs that already call OpenAI's `/v1/images/generations`
or `/v1/images/edits` can point at AIGateway unchanged.
...
(`gpt-image-1` always returns base64). AIGateway adds a `provider` field naming the
...
- **Token-billed models** (`gpt-image-1`, Gemini image models) report `usage`.
  AIGateway stores the token counts and prices the generated tokens with the
...
- **Per-image models** (DALL·E, `grok-imagine-image`, Imagen) report no tokens.
  AIGateway records the number of returned images (`images` in the raw usage
