---
title: "Configuration"
description: "Configure AIGateway with environment variables, .env files, or config.yaml, and see which layer wins. Every setting has a default, so zero config works."
...
AIGateway uses a good defaults philosophy. This means that the default settings should be enough to use it.
...
<Tip>
  As AIGateway works out of the box with no configuration files, you can try it in
  a minute.
...
AIGateway automatically discovers providers from well-known environment variables.

## Configuration Methods

### 1. Environment Variables

The most common way to configure AIGateway. Set any of the variables below to override defaults.
...
| `DOCS_DIR`           | Directory containing the exported product docs       | _(embedded fallback)_ |
| `AIGATEWAY_MASTER_KEY` | Authentication key for securing the gateway           | _(empty, unsafe mode)_ |
| `AIGATEWAY_DEMO_MODE`  | Enable public demo warnings in logs and the dashboard  | `false`                |
...
| `USER_PATH_HEADER`   | Header used to read/write request `user_path` values  | `X-AIGateway-User-Path`  |
...
| `PID_FILE`           | Where the running gateway records its process id for `aigateway --reload`. Changing it needs a restart | `data/aigateway.pid` next to a `./data` directory, otherwise the per-user data directory |
...
Set `AIGATEWAY_DEMO_MODE=true` for a public or shared demonstration instance.
AIGateway logs a warning at startup and every five minutes, renders a persistent
...
`AIGATEWAY_MASTER_KEY` is empty. Disabled or expired managed keys keep
...
| Variable            | Description                       | Default          |
| ------------------- | --------------------------------- | ---------------- |
| `AIGATEWAY_CACHE_DIR` | Directory for local cache files   | `.cache`         |
...
| `MODEL_LIST_URL`    | Model metadata catalog: an HTTP URL, a local file path (`/etc/aigateway/models.json` or `file://...`), or `off` | public catalog on GitHub |
| `AIGATEWAY_OFFLINE`   | Air-gap switch: disables the update check and drops HTTP catalog URLs. Local catalog files and configured providers are unaffected | `false` |
| `REDIS_URL`         | Redis connection URL              | _(empty)_        |
| `REDIS_KEY_MODELS`     | Redis key for model cache           | `aigateway:models`    |
| `REDIS_KEY_RESPONSES`  | Redis key for response cache        | `aigateway:response:` |
...
| `SQLITE_PATH`        | SQLite database file path                     | `data/aigateway.db` |
...
| `MONGODB_DATABASE`   | MongoDB database name; overrides the URL path (alias: `MONGO_DATABASE`) | `aigateway`         |
...
<Note>
  ...
  bundled `pg_trgm` extension. AIGateway installs the extension and builds the
...
<Note>
  ...
  both on, every rewriter that changed a request (for example AIGateway Pro token
...
Budgets use tracked usage cost records. If usage tracking is disabled, AIGateway
...
`USAGE_ENABLED=false`, AIGateway starts with budgets disabled and logs a warning,
...
Most providers can use a custom base URL via `<PROVIDER>_BASE_URL` (for example `OPENAI_BASE_URL`). Chutes AI defaults to `https://llm.chutes.ai/v1` and can be overridden with `CHUTES_BASE_URL`. DeepSeek defaults to `https://api.deepseek.com`; set `DEEPSEEK_BASE_URL` only for a compatible proxy or alternate DeepSeek endpoint. OpenRouter defaults to `https://openrouter.ai/api/v1` and can be overridden with `OPENROUTER_BASE_URL`. Kilo AI defaults to `https://api.kilo.ai/api/gateway` and can be overridden with `KILO_BASE_URL`. Z.ai defaults to `https://api.z.ai/api/paas/v4`; set `ZAI_BASE_URL=https://api.z.ai/api/coding/paas/v4` for the GLM Coding Plan endpoint. SGLang defaults to `http://localhost:30000/v1` when `SGLANG_API_KEY` is set, but keyless deployments should set `SGLANG_BASE_URL` explicitly to register the provider. vLLM follows the same pattern at `http://localhost:8000/v1`. llama.cpp's `LLAMACPP_BASE_URL` is always required (llama-server's default port collides with AIGateway's own 8080, so there is no default); `LLAMACPP_API_KEY` is optional. llm-d has no universal endpoint, so `LLMD_BASE_URL` is always required; `LLMD_API_KEY` is optional. Azure uses `AZURE_BASE_URL` for its deployment base URL and accepts an optional `AZURE_API_VERSION` override; otherwise it defaults to `2024-10-21`. Oracle requires `ORACLE_BASE_URL` because its OpenAI-compatible endpoint is region-specific.
...
For OpenRouter, AIGateway also sends default attribution headers unless the request already sets them. Override those defaults with `OPENROUTER_SITE_URL` and `OPENROUTER_APP_NAME`.

### 2. `.env` File

AIGateway automatically loads a `.env` file from the working directory at startup. This is convenient for local development.
...
For more complex setups, you can use an optional YAML configuration file. AIGateway looks for it in two locations (in order):
...
server:
  port: "3000"
  base_path: "/g"
  user_path_header: "X-AIGateway-User-Path"
...
The simplest way to add providers. AIGateway checks for well-known API key environment variables and automatically registers providers:
...
type without YAML. AIGateway normalizes the suffix to lowercase and converts
...
providers:
  ...
  # Add DeepSeek. AIGateway translates /v1/responses to DeepSeek chat completions.
  deepseek:
    type: deepseek
...
omits it, AIGateway injects a fallback so the request still succeeds. OpenAI and
Gemini treat the field as optional and AIGateway does not inject a default for
them.
