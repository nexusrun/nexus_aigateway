...
embeddings, image, or audio model). AIGateway assembles it from five sources,
strongest first:
...
4. **The model catalog** — the
   [`ai-model-list`](https://github.com/nexusrun/ai-model-list) registry,
...
```bash
MODEL_LIST_URL=/etc/aigateway/models.json          # bare path
MODEL_LIST_URL=file:///etc/aigateway/models.json   # file URL
```
...
`AIGATEWAY_OFFLINE=true`, which drops HTTP catalog URLs. Set `MODEL_LIST_URL=off`
