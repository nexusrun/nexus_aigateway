...
description: "Create, read, and delete OpenAI-compatible conversations and their items through AIGateway, which stores them at the gateway so any provider can be used."
...
AIGateway exposes OpenAI-compatible Conversations API endpoints under
`/v1/conversations`.

Conversations are a **gateway-managed resource**. The OpenAI Conversations API
is provider-specific, so AIGateway generates the conversation ID and stores the
...
AIGateway enforces the OpenAI Conversations limits so the public contract stays
compatible:
...
Item payloads follow the Responses API input/output item union. AIGateway
...
`message.output_text.logprobs`. AIGateway stores complete item payloads, so
optional fields remain available for later requests while only the fields named
by `include` are returned.

## Storage and retention

The normal AIGateway application stores conversations in the configured shared
...
AIGateway returns OpenAI-compatible errors:
...
`response.completed`. AIGateway does not currently deduplicate a client retry of
