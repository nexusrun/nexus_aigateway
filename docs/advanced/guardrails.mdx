...
so AIGateway renumbers what it delivers: `sequence_number` still runs from `0`
...
<img
  ...
  alt="AIGateway dashboard Plugins &amp; Guardrails page with an empty Guardrails list, a Create Guardrail button, and the loaded plugin list"
...
Six types ship with AIGateway. Each is a built-in plugin; the tables list its
...
| `response_remove` | Drop a header from the client response, including headers AIGateway adds itself (for example `X-Request-Id`). |
...
needed; AIGateway rewrites the text itself, so it can put the original values
...
| **warn** | The unchanged response plus an `X-AIGateway-Guardrail: warn; code=<code>` header; the detail is stored in the audit trail. On a stream the header is sent only when the warn was decided before the first bytes went out (a buffered response); later warns are audit-only. |
