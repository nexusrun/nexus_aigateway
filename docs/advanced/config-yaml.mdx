...
The goal in AIGateway is to make `config.yaml` optional as much as possible.
...
To confirm which layer supplied your providers, AIGateway logs one line at boot:
...
providers. AIGateway also logs `config file loaded` with the resolved path, or
...
- [config/config.go](https://github.com/nexusrun/nexus_aigateway/blob/main/config/config.go)
- [config/config.example.yaml](https://github.com/nexusrun/nexus_aigateway/blob/main/config/config.example.yaml)
...
`USER_PATH_HEADER`. The default remains `X-AIGateway-User-Path`.

Edits to this file are applied to a running gateway with `aigateway --reload`,
...
AIGateway reads `config/config.yaml` first, then `config.yaml`.

With `docker run`, you can keep a host file named `config.yml` and mount it to
the in-container path AIGateway expects:

```bash
docker run --rm -p 8080:8080 \
  -v "$PWD/config.yml:/app/config/config.yaml:ro" \
  nexusrun/nexus_aigateway:latest
```
...
services:
  aigateway:
    image: nexusrun/nexus_aigateway:latest
    ports:
      - "8080:8080"
    volumes:
