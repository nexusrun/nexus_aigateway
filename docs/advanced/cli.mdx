...
description: "Command-line flags for inspecting the AIGateway binary, probing a running gateway's health, and reloading its configuration."
icon: "terminal"
...
The `aigateway` binary exposes a small set of command-line flags for operational
tasks. Flags accept both single-dash (`-flag`) and double-dash (`--flag`) forms;
the examples below use the long form.

| Flag               | Description                                              | Default |
| ------------------ | ------------------------------------------------------- | ------- |
| `--version`        | Print version information and exit                       | N/A |
| `--health`         | Probe the local `/health` (liveness) endpoint and exit  | N/A |
| `--health-timeout` | Maximum time to wait for the `--health` probe            | `2s`    |
| `--ready`          | Probe the local `/health/ready` (readiness) endpoint and exit | N/A |
| `--ready-timeout`  | Maximum time to wait for the `--ready` probe            | `4s`    |
| `--reload`         | Tell the running gateway to reload its configuration and exit | N/A |
...
```bash
aigateway --health --health-timeout 5s
```
...
```bash
kill -HUP "$(cat data/aigateway.pid)"
```
...
and removes it on shutdown. The default is `data/aigateway.pid` when a `./data`
...
Both the gateway and `aigateway --reload` resolve the path from the same
configuration, so run the command from the same working directory (or with the
same `PID_FILE`) as the gateway:

```bash
docker exec my-gateway /aigateway --reload
```
...
- **`AIGATEWAY_DEMO_MODE`**: the demo warnings are wired up once at startup.
...
`aigateway --ready` as an exec probe. Keep `--ready-timeout` larger than the
...
`aigateway plugin` builds and inspects shared-object plugins (`.so`) with the
toolchain of the running binary; see [Plugins](/advanced/plugins#shared-object-plugins).

```sh
aigateway plugin build [-o out.so] <dir>   # go build -buildmode=plugin with this binary's flags
aigateway plugin inspect <file.so>         # open the file and print its manifest and build info
```

`plugin build` forces `CGO_ENABLED=1`, pins `GOTOOLCHAIN` to the host's Go
version, stamps a `AIGatewayBuildInfo` variable into the plugin, and refuses an
output built with a different Go version. Loading needs a cgo build of
AIGateway (`make build-plugins` or the `aigateway:<version>-plugins` image).
`aigateway --version` prints the Go and `pluginapi` versions plugins must match.
