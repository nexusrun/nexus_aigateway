...
AIGateway uses it to keep model access, workflows, budgets, usage, and audit data
...
<img
  ...
  alt="AIGateway dashboard API Keys page with a User Path column showing values like /agents/team1, /engineering/ai, and /sales/john"
...
When a request uses that key, AIGateway treats the key's `user_path` as the
...
```http
X-AIGateway-User-Path: /team/alpha
```

The header name is configurable with `USER_PATH_HEADER` or
`server.user_path_header` in `config.yaml`. If unset, AIGateway uses
`X-AIGateway-User-Path`.

If the API key has its own `user_path`, the key wins. AIGateway overwrites the
