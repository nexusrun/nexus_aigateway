...
description: "Use AIGateway virtual models to expose stable model names and test model swaps without changing app code."
icon: "tags"
...
<img
  ...
  alt="AIGateway dashboard models page with the New virtual model button and an active redirect row"
...
AIGateway picks the kind from whether you give it a target:

- A **redirect** (an alias) is a stable name that points to a concrete provider
  model. Your app sends the name in the `model` field, and AIGateway resolves it
...
AIGateway does not persist empty, ineffective access policies, whether saved from
...
access state, AIGateway removes the stored row. It retains an otherwise empty
...
For Chat Completions and Responses API SSE streams, AIGateway drains the upstream
...
- **Adaptive** (AIGateway Pro) picks the target with the best observed latency,
...
Open a redirect in the dashboard and edit its **Source** to rename it. AIGateway
...
When this is enabled, AIGateway returns enabled redirects from `/v1/models`
...
Your app can keep sending `anthropic/claude-opus-4-6`, while AIGateway routes the
