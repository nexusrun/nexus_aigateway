...
request was sent to returns a failover-eligible error, AIGateway retries the
...
- `5xx`
- `429`, including a rate limit AIGateway itself enforces on the target — a
...
counted in `aigateway_empty_responses_total`.
