...
| Piece                   | Source                                                       |
| ----------------------- | ------------------------------------------------------------ |
| the request's user path | the managed API key's `user_path`, an SSO session, or the `X-AIGateway-User-Path` header |
...
`Users` lists every node with its own allowlist and its **effective models**,
