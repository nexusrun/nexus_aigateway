---
title: "MCP Gateway"
description: "Aggregate your MCP servers behind one authenticated AIGateway endpoint with per-key tool visibility, usage tracking, and rate limits."
icon: "plug"
...
AIGateway can sit between your agent clients (Claude Code, Claude Desktop,
...
- **One endpoint, one key.** Clients connect to `https://your-gateway/mcp`
  with a AIGateway API key. They never hold upstream MCP credentials; the
...
Point any streamable-HTTP MCP client at the gateway and pass your AIGateway key
as a bearer token:
...
```json
{
  "mcpServers": {
    "aigateway": {
      ...
      "headers": {
        "Authorization": "Bearer sk-your-aigateway-key"
      }
    }
  }
}
...
| --- | --- | --- |
| `url` | N/A | Endpoint for `http` (streamable HTTP) and `sse` transports |
| `transport` | `http` | `http`, `sse` (legacy), or `stdio` (config-only) |
| `headers` | N/A | Sent verbatim upstream; values support `${ENV}` |
| `command`, `args`, `env` | N/A | stdio subprocess definition |
...
- **Credential boundary.** The MCP spec forbids token passthrough; AIGateway
