...
[label](/features/labelling). AIGateway evaluates them from tracked usage cost
...
If usage tracking is disabled, AIGateway starts with budget management disabled
and logs a warning.
...
<img
  ...
  alt="AIGateway dashboard Budgets page listing per-user-path monthly, weekly, and daily budgets with spend bars and remaining amounts"
    ...
    - path: "/team/alpha"
      limits:
        ...
        - period: "weekly"
          amount: 50.00
    - path: "/customers"
      # AIGateway Pro: requires the quota_templates entitlement.
      per_child: true
      limits:
        - period: "daily"
          amount: 5.00
...
# AIGateway Pro (quota_templates entitlement):
...
If multiple budgets match a request, AIGateway checks all matching limits in one
lookup. The request is rejected if any matching limit is exhausted.

### Dynamic per-child budgets

<Info>
  Dynamic per-child budgets are available in AIGateway Pro and require the
  ...
  configuration aborts startup — AIGateway never silently falls back to one
...
<Note>
  ...
  exists, AIGateway can serve it without spending additional provider cost.
</Note>
...
```bash curl
curl -H "Authorization: Bearer $AIGATEWAY_MASTER_KEY" \
...
client = OpenAI(
    ...
    api_key=os.environ["AIGATEWAY_MASTER_KEY"],
)
...
const client = new OpenAI({
  ...
  apiKey: process.env.AIGATEWAY_MASTER_KEY,
});
