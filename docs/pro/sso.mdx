...
description: "Protect the AIGateway Dashboard with your OIDC identity provider using Authorization Code flow with PKCE."
icon: "key-round"
tag: "Pro"
keywords: ["AIGateway Pro", "OIDC", "SSO", "single sign-on", "PKCE", "dashboard authentication"]
---
...
Configure it under `extensions.sso` in the main AIGateway YAML configuration:
...
extensions:
  sso:
    ...
    redirect_url: https://aigateway.example.com/sso/callback
    session_secret: ${PRO_SSO_SESSION_SECRET}
    admin_groups: [aigateway-admins]
```
...
<Warning>
  ...
  AIGateway Pro replica until a shared session or revocation store is available.
</Warning>
