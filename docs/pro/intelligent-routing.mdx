...
icon: "route"
tag: "Pro · Beta"
keywords: ["AIGateway Pro", "intelligent routing", "smart routing", "model tiers", "adaptive routing", "load balancing", "cost optimization"]
---
...
Configure a tiered alias with ordinary AIGateway model names. The tier targets can
...
| `PRO_SMART_ROUTING_ROUTES` | N/A | JSON map from requested aliases to `easy` and `hard` models. No routes leaves the feature idle. |
...
Send `X-AIGateway-Smart-Routing: off` to bypass classification for one request,
or `X-AIGateway-Tier: easy` or `X-AIGateway-Tier: hard` to force its tier without
changing the session pin.

Every routed response reports the decision:

```http
X-AIGateway-Pro-Tier: hard
X-AIGateway-Pro-Tier-Target: normal-hard
X-AIGateway-Pro-Tier-Score: 5
```
...
`aigateway_pro_smartroute_` prefix; the score histogram is the tool for tuning
...
`aigateway_pro_routing_session_affinity_total{outcome}` counts pins `honored`
and pins `released` to a cooldown, so the behaviour is visible in Prometheus
rather than only in a provider invoice.

Prometheus metrics use the `aigateway_pro_routing_` prefix (selections,
