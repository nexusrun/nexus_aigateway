---
title: "ElevenLabs"
description: "Configure ElevenLabs in AIGateway: voice_id vs named voices, supported audio formats, and speech-to-text timestamps."
icon: "audio-lines"
...
work: AIGateway's provider-passthrough router treats a leading `v1/` segment in
...
ElevenLabs' `/v1/models` listing, AIGateway adds them to `/v1/models` output
...
which AIGateway maps into the OpenAI `words` array. `prompt` is not supported.
...
AIGateway only implements the two ElevenLabs capabilities that map onto
OpenAI-compatible endpoints: text-to-speech and speech-to-text. Everything
else ElevenLabs offers has no typed support in AIGateway today:
...
- **Speech-to-speech (voice changer)**: `POST /v1/speech-to-speech/{voice_id}`
  ...
  around it with, so it is entirely unreachable through AIGateway right now.
...
- **Realtime/streaming TTS and STT** (`/v1/text-to-speech/{voice_id}/stream`,
  WebSocket streaming), AIGateway's `/v1/audio/speech` and
...
AIGateway's audio surface is deliberately OpenAI-shaped, and ElevenLabs' API is
