---
title: "Xiaomi MiMo"
description: "Configure Xiaomi MiMo in AIGateway: thinking mode, the [1m] context suffix, and how TTS/ASR map onto the standard audio endpoints."
icon: "mic"
...
AIGateway translates the standard `/v1/audio/speech` and `/v1/audio/transcriptions`
...
request with MiMo's `thinking` parameter — AIGateway forwards it unchanged:
...
AIGateway preserves it in both directions.
...
(`mimo-v2.5-asr`) run through chat completions. AIGateway exposes both ways:
...
- **MiMo's chat dialect** — send chat completions directly: synthesis text in
  ...
  transcription. AIGateway forwards these shapes untouched.
