...
AIGateway calls its Responses API directly, so Responses-only models are not
...
For SigV4, AIGateway uses the AWS SDK credential chain, including
...
For an API-key or YAML configuration where the base URL is omitted, AIGateway
...
Invoke. AIGateway automatically sends them to `/openai/v1/responses`, the model-
...
`BEDROCK_MANTLE_API_MODE` controls which OpenAI-compatible path AIGateway uses:
...
model before AIGateway's automatic path list is updated.
