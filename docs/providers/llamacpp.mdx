---
title: "llama.cpp"
description: "Route OpenAI-compatible AIGateway requests to a llama.cpp llama-server instance (the same provider type works for LM Studio)."
icon: "server"
keywords: ["llama.cpp", "llama-server", "LM Studio", "local models", "self-hosted", "OpenAI-compatible server"]
---

llama.cpp's `llama-server` speaks the OpenAI API directly, and AIGateway has a
...
port (8080) collides with AIGateway's own, so there is no default:
...
<Note>
  These examples assume AIGateway runs in Docker and llama-server is on the host
  at `localhost:8081`, hence `host.docker.internal`. If AIGateway runs on the
...
`/v1/embeddings` works through AIGateway as long as llama-server can serve it:
...
Local GGUFs are not in the upstream model catalog, so AIGateway reads what it can
...
so AIGateway classifies its models by ID: names containing `embed` or matching
...
- **Reranking**: llama-server serves reranking (start with a reranker model
  and `--rerank --embedding --pooling rank`), but AIGateway has no rerank
