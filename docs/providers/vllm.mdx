---
title: "vLLM"
description: "Route OpenAI-compatible AIGateway requests to one or more vLLM servers, including slash-shaped Hugging Face model IDs."
icon: "server"
keywords: ["vLLM", "self-hosted", "Hugging Face models", "OpenAI-compatible server"]
---

AIGateway talks to vLLM through its OpenAI-compatible HTTP server. Hugging Face
model IDs with slashes (e.g. `meta-llama/Llama-3.1-8B-Instruct`) work — AIGateway
...
<Note>
  These examples assume AIGateway runs in Docker and vLLM is on the host at
  `localhost:8000` — hence `host.docker.internal`. If both run in the same
  Docker network, use the vLLM service name. If AIGateway runs on the host
...
AIGateway strips client auth headers before forwarding and applies `VLLM_API_KEY`
