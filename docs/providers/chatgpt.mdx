...
`Codex -> AIGateway -> ChatGPT subscription`.
...
Run `codex login` first if `~/.codex/auth.json` does not exist yet. AIGateway
...
The Codex backend has no model-listing endpoint, so AIGateway ships the
...
The backend also validates against a strict parameter allowlist. AIGateway
...
<Warning>
  ...
  your own Codex usage through AIGateway, not as a general-purpose inference API.
</Warning>
