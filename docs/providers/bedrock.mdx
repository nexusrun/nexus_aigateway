...
description: "Configure Amazon Bedrock in AIGateway using AWS credentials and the Bedrock Runtime Converse API."
icon: "/brand/bedrock.svg"
keywords: ["Amazon Bedrock", "AWS", "Converse API", "provider setup"]
---

AIGateway routes Bedrock requests through the Bedrock Runtime Converse and
...
Bedrock has no API key of its own, AIGateway resolves AWS credentials at runtime
...
long-lived access keys. When `BEDROCK_BASE_URL` is empty, AIGateway falls back to
...
By default AIGateway queries Bedrock's control plane and lists all on-demand
...
- **Wrong region inferred**: pass `BEDROCK_BASE_URL` explicitly. With a full
  endpoint URL, AIGateway only extracts the region segment when the host ends in
