---
title: "xAI (Grok)"
description: "Configure xAI Grok models in AIGateway and understand how reasoning effort and prompt-cache affinity are handled."
icon: "x"
...
`reasoning_effort` as a top-level string on Chat Completions. AIGateway rewrites
...
parameter reasoningEffort`. AIGateway drops the field for them instead of
...
field, so a new reasoning model works before AIGateway learns about it.
...
- **Chat Completions:** AIGateway forwards a client-supplied `X-Grok-Conv-Id`
...
- **Responses API:** pass `prompt_cache_key` in the request body; AIGateway
  forwards it verbatim.

## `metadata` on the Responses API

xAI's native `/responses` endpoint rejects the standard OpenAI `metadata`
member with `400 "Argument not supported: metadata"`. AIGateway drops it from
