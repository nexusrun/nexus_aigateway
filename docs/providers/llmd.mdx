---
title: "llm-d"
description: "Route AIGateway requests through the llm-d Inference Gateway and inject trusted scheduling metadata."
icon: "route"
keywords: ["llm-d", "Inference Gateway", "EPP", "Kubernetes", "inference scheduling"]
---

AIGateway's `llmd` provider sends OpenAI-compatible requests to an
...
when AIGateway is the authenticated API gateway in front of llm-d.

## Prerequisites

- A running llm-d deployment with an HTTPRoute that reaches the Router/EPP.
- The model name exposed by that route.
- Network access from AIGateway to the Router service.
...
When `LLMD_INFERENCE_OBJECTIVE` is omitted, AIGateway does not send an inference
objective header. Set it when the llm-d deployment defines multiple inference
objectives and this AIGateway provider should select one of them.

Declare `LLMD_MODELS` when your route does not serve `GET /v1/models`. AIGateway
...
`fairness_from_user_path` defaults to `true`. AIGateway derives the fairness ID
...
<Note>
  A raw `X-AIGateway-User-Path` supplied by a client is not enough to set the
  fairness ID. Bind the path to a managed auth key or an extension identity so
  AIGateway can treat it as an effective authenticated path.
</Note>
...
For the objective and fairness ID, AIGateway sends both the current `x-llm-d-*`
...
When EPP flow control returns a `429` on a translated inference route, AIGateway
...
First create a managed AIGateway API key in the dashboard, bind its `user_path`
to `/team/alpha`, and assign the returned value to `TEAM_ALPHA_AIGATEWAY_KEY`.
...
curl -s http://localhost:8080/v1/chat/completions \
  -H "Authorization: Bearer ${TEAM_ALPHA_AIGATEWAY_KEY}" \
...
client = OpenAI(
    ...
    api_key=os.environ["TEAM_ALPHA_AIGATEWAY_KEY"],
)
...
const client = new OpenAI({
  ...
  apiKey: process.env.TEAM_ALPHA_AIGATEWAY_KEY,
});
...
AIGateway removes the outer `llmd/` routing qualifier before sending the model
...
The separate llm-d Batch Gateway is not exposed as AIGateway's native batch API.
