...
DashScope in consoles, SDKs, and API documentation. AIGateway routes to it through
the OpenAI-compatible endpoint (`/compatible-mode/v1`).

<Note>
  AIGateway keeps the provider ID, environment variables, and passthrough path as
  `bailian`, `BAILIAN_*`, and `/p/bailian/...` for compatibility.
</Note>

Because Bailian deprecated `max_tokens` in April 2026 in favor of
`max_completion_tokens`, AIGateway automatically maps the standard
...
AIGateway transparently maps the standard `max_tokens` parameter to
`max_completion_tokens` for every `bailian` provider request, send
`max_tokens` as you normally would, and AIGateway rewrites it before forwarding to
Model Studio.
