---
title: "Google Gemini"
description: "Configure Google Gemini via AI Studio in AIGateway, choose native or OpenAI-compatible routing, and understand image_url behavior."
icon: "sparkles"
...
By default, AIGateway routes Gemini chat, Responses, embeddings, and
...
Native AI Studio mode can also materialize AIGateway's post-routing prompt-cache
...
AIGateway keeps separate internal bases for native Gemini and the
...
`GEMINI_BASE_URL` overrides them. When the value ends in `/openai`, AIGateway
...
Flash reject `minimal`, so AIGateway sends the lowest level those models accept
...
and list `candidate_count` as unsupported on Gemini 3.x. In native mode AIGateway
...
native mode AIGateway exposes the signature the same way Google's own
...
AIGateway sends Google's documented `skip_thought_signature_validator`
