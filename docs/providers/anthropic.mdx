---
title: "Anthropic"
description: "Configure Anthropic (Claude) in AIGateway and understand how reasoning effort maps to Claude's adaptive thinking and effort control."
icon: "sparkles"
...
AIGateway maps the OpenAI-style `reasoning.effort` knob onto Claude's native
...
<Note>
  Anthropic's `/v1/messages` requires `max_tokens` on every request. AIGateway
...
AIGateway also accepts a Claude subscription OAuth token as the Anthropic
...
Tokens with the `sk-ant-oat` prefix are detected automatically: AIGateway sends
...
<Warning>
  ...
  traffic**. Use this to route your own Claude Code through AIGateway (see the
...
AIGateway accepts the OpenAI-shaped `"reasoning": {"effort": "..."}` object as
...
<Note>
  Omit `reasoning` to leave thinking at the model's default. AIGateway only sets
...
<Note>
  ...
  Sonnet 4.6. AIGateway forwards the level you send; Anthropic rejects it with a
  400 if the target model does not support it. Manual `budget_tokens` thinking
  is rejected from Opus 4.7 onward, which is why AIGateway uses adaptive thinking
  for those models.
</Note>
...
default of `temperature: 1`, is rejected upstream with a 400. AIGateway drops
...
clients routinely fill in both defaults, AIGateway forwards `temperature` and
...
requires `temperature = 1`. AIGateway drops any other temperature value (and logs
...
from Anthropic. AIGateway follows Anthropic's documented replacement: the choice
