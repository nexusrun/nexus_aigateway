---
title: "DeepSeek"
description: "Configure DeepSeek V4 in AIGateway and understand how reasoning effort is mapped to DeepSeek's reasoning_effort field."
icon: "brain"
keywords: ["DeepSeek", "reasoning effort", "provider setup"]
---

DeepSeek does not expose a native Responses API, so AIGateway translates
...
| `xhigh` / `max` | `max` |
| anything else | passed through by AIGateway, then rejected by DeepSeek |

AIGateway rewrites the OpenAI-shaped `"reasoning": {"effort": "..."}` into
DeepSeek's top-level `reasoning_effort`, no client change required. Validate
or normalize custom client values to `high` or `max` before calling AIGateway.
