---
title: "OpenCode Go"
description: "Configure OpenCode Zen (Go subscription) in AIGateway using an OpenAI-compatible endpoint."
icon: "terminal"
keywords: ["OpenCode Zen", "OpenCode Go", "provider setup"]
---

[OpenCode Zen](https://opencode.ai/docs/zen/) exposes its "Go" subscription
models behind an OpenAI-compatible API. AIGateway ships a native `opencode_go`
...
(`x-api-key` auth). AIGateway routes per model:
...
- **Anthropic-only models** are translated to the Anthropic Messages dialect and
  sent to `/messages`, mirroring how AIGateway already serves native Anthropic.
...
AIGateway adds the header to every OpenCode Go request by default, on both the
...
2. Otherwise the session AIGateway detected through
...
A request with no detectable session is sent without the header. AIGateway also
identifies itself with `x-opencode-client` (forwarding the client's own value
when present, `aigateway` otherwise) and a `aigateway/<version>` User-Agent, as
...
AIGateway therefore sends `reasoning_effort: "low"` on `/chat/completions` when a
...
over an explicit `reasoning.effort` or a top-level `reasoning_effort`. AIGateway's
...
`reasoning.effort` is AIGateway's canonical field, so a request that sets both it
and a top-level `reasoning_effort` is sent with the mapped `reasoning.effort` —
the same precedence every other AIGateway provider applies.
