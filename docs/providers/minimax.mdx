---
title: "MiniMax"
description: "Configure MiniMax in AIGateway: chat models, temperature handling, and native text-to-speech through the standard audio endpoint."
icon: "audio-waveform"
...
box. Text-to-speech, however, uses MiniMax's own `t2a_v2` API — AIGateway
...
MiniMax requires `temperature` in `(0.0, 1.0]` and rejects zero. AIGateway clamps
...
MiniMax reports failures as HTTP 200 with a native status code; AIGateway maps
