...
AIGateway has two observability outputs:

- **Prometheus** (`METRICS_ENABLED`): the `aigateway_*` gateway metrics, defined
  with `prometheus/client_golang` in `internal/observability` and pulled from
  `METRICS_ENDPOINT`. AIGateway Pro adds `aigateway_pro_*` metrics the same way.
...
have changed across releases. Building AIGateway's scrape surface on it would
mean tracking those changes while AIGateway's own Prometheus surface is still
...
- OpenTelemetry emits semantic-convention signals only; it is not a second
  transport for `aigateway_*` metrics.
...
- Operators who want both `aigateway_*` and `gen_ai.*` in one backend route the
...
The OTel Prometheus exporter reaches a stable release, or AIGateway's Prometheus
...
and an OTLP reader under `OTEL_ENABLED`, preserving the existing `aigateway_*`
